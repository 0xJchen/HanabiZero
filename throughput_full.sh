...
python3 main.py --env Hanabi-Full --case hanabi --opr train --seed 1 --num_gpus 2 --num_cpus 30 --force --batch_actor 4\
  --p_mcts_num 4\
  --extra full_2p_test_with_ray\
  --use_priority \
  --use_max_priority \
  --revisit_policy_search_rate 0.99 \
  --amp_type 'torch_amp' \
  --reanalyze_part 'none' \
  --info 'full_partial_no_reanalyze' \
  --val_coeff 1 \
  --td_steps 10
#simulation=200
