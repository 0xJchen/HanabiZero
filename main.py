...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='MuZero Pytorch Implementation')
    ...
    parser.add_argument('--extra', default='none', type=str,
                        help='extra information in log directory')
    ...
    parser.add_argument('--opr', required=True, choices=[
                        'train', 'test', 'pytest', 'make_dataset', 'super', 'debug', 'mcts'])
    ...
    parser.add_argument('--batch_actor', type=int,
                        default=1, help='batch actor')
    parser.add_argument('--replay_number', type=int,
                        default=1, help='Add batch buffer')
    parser.add_argument('--p_mcts_num', type=int, default=64,
                        help='number of parallel mcts')
    parser.add_argument('--seed', type=int, default=0,
                        help='seed (default: %(default)s)')
    parser.add_argument('--num_gpus', type=int,
                        default=8, help='gpus available')
    ...
    parser.add_argument('--val_coeff', type=float,
                        default='0.25', help='val coeff')
    parser.add_argument('--lr', type=float, default='0.1',
                        help='learning rate')
    parser.add_argument('--td_steps', type=int, default=5, help='td step')
    parser.add_argument('--const', type=float,
                        default='0.', help='consistent loss')
    parser.add_argument('--actors', type=int, default=2, help='actors')
    parser.add_argument('--simulations', type=int,
                        default=50, help='simulation')
    parser.add_argument('--batch_size', type=int,
                        default=256, help='batch size')
    parser.add_argument('--debug_batch', action='store_true',
                        default=False, help='show batch statistics')
    # Process arguments
    parser.add_argument('--debug_interval', type=int,
                        default=1000, help='show batch time interval')
    parser.add_argument('--stack', type=int, default=1, help='stacked frame')
    parser.add_argument('--decay_step', type=int,
                        default=200000, help='decay step')
    parser.add_argument('--decay_rate', type=float,
                        default=1, help='decay rate')
    parser.add_argument('--test_start', type=int, default=1,
                        help='start test model index')
    parser.add_argument('--test_end', type=int, default=2,
                        help='end test model index')
    ...
    parser.add_argument('--optim', required=True, choices=['adam', 'sgd', 'rmsprop'],
                        help='optimizer to use')
    parser.add_argument('--obj_store_size', type=int, default=100,
                        help='object store siez in GB')
    #snapshot
    parser.add_argument('--save_snapshot', type=int, default=0,
                        help='save snapshot steps')
    parser.add_argument('--load_snapshot', type=str,
                        default='none', help='load snapshot path')

    parser.add_argument('--local_state', action='store_true', default=False,
                        help='currently debugging with global state')
    ...
    if args.opr == 'train':
        ray.init(num_gpus=args.num_gpus, num_cpus=args.num_cpus,
                 object_store_memory=args.obj_store_size*1024*1024*1024, dashboard_port=8265, dashboard_host='0.0.0.0')
    ...
    try:
        if args.opr == 'train':
            ...
            print("paht exististence ", args.model_path, os.path.exists(args.model_path))
            if args.load_model:
            ...
            logging.getLogger('test').info(
                'Test Mean Score: {}'.format(mean_score))
            ...
            for idx in test_range:
                ...
                test_score, test_path = test(muzero_config, model, 0, args.test_episodes,
                                             device='cuda', render=False, save_video=False, final_test=True)
                mean_score = sum(test_score) / len(test_score)
                print("model: {}, mean: {}".format(
                    idx*10000, mean_score), flush=True)

            model, weights = test_mcts(
                muzero_config, summary_writer, model_path)
