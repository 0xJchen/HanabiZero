...
cdef extern from "cnode.h" namespace "tree":
    cdef cppclass CNode:
        ...
        void expand(int to_play, int hidden_state_index_x, int hidden_state_index_y, float reward, vector[float] policy_logits, vector[int] &stackLegalActions)
        void add_exploration_noise(float exploration_fraction, vector[float] noises, vector[int] legalActions)
    ...
    cdef cppclass CRoots:
        ...
        void prepare(float root_exploration_fraction, const vector[vector[float]] &noises, const vector[float] &rewards, const vector[vector[float]] &policies, const vector[vector[int]] &stackLegalActions)
        void prepare_no_noise(const vector[float] &rewards, const vector[vector[float]] &policies, const vector[vector[int]] &stackLegalActions)
        void clear()
    ...
    void cmulti_back_propagate(int hidden_state_index_x, float discount, vector[float] rewards, vector[float] values, vector[vector[float]] policies,
