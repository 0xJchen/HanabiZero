...
namespace tree
{
    ...
    void CNode::print_out()
    {
        ...
        printf("visit count: %d \t hidden_state_index_x: %d \t hidden_state_index_y: %d \t reward: %f \t prior: %f \n.",
               this->visit_count, this->hidden_state_index_x, this->hidden_state_index_y, this->reward, this->prior);
    ...
    void CRoots::prepare(float root_exploration_fraction, const std::vector<std::vector<float>> &noises, const std::vector<float> &rewards, const std::vector<std::vector<float>> &policies, const std::vector<std::vector<int>> &stackLegalActions)
    {
        for (int i = 0; i < this->root_num; ++i)
        {
            this->roots[i].expand(0, 0, i, rewards[i], policies[i], stackLegalActions[i]);
            this->roots[i].add_exploration_noise(root_exploration_fraction, noises[i], stackLegalActions[i]);
        }
    ...
    void CRoots::prepare_no_noise(const std::vector<float> &rewards, const std::vector<std::vector<float>> &policies, const std::vector<std::vector<int>> &stackLegalActions)
    {
        for (int i = 0; i < this->root_num; ++i)
        {
            this->roots[i].expand(0, 0, i, rewards[i], policies[i], stackLegalActions[i]);
    ...
    {
        int numAction = policies[0].size();
        std::vector<int> legalActions(numAction, 1);
        for (int i = 0; i < results.num; ++i)
        {
            results.nodes[i]->expand(0, hidden_state_index_x, i, rewards[i], policies[i], legalActions);
    ...
    int cselect_child(CNode *root, tools::CMinMaxStats &min_max_stats, int pb_c_base, float pb_c_init, float discount, float mean_q)
    {
        ...
        for (int a = 0; a < root->action_num; ++a)
        {
            ...
            float temp_score = cucb_score(child, min_max_stats, mean_q, root->visit_count, pb_c_base, pb_c_init, discount);
    ...
    float cucb_score(CNode *child, tools::CMinMaxStats &min_max_stats, float parent_mean_q, float parent_visit_count, float pb_c_base, float pb_c_init, float discount)
    {
        ...
        // manually filter impossible child prior
