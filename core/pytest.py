...
def test(config, model, counter, test_episodes, device, render, save_video=False, final_test=False):
    ...
    with torch.no_grad():
        ...
        while not dones.all():
            ...
            action_histories=[[]for _ in range(root_num)]
            ...
            roots_distributions = np.array(py_roots_distributions)
            roots_values = np.array(py_roots_values)
            #==============================================================================================
