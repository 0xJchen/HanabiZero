...
def test(config, model, counter, test_episodes, device, render, save_video=False, final_test=False):
    ...
    save_path = os.path.join(
        config.exp_path, 'recordings', 'step_{}'.format(counter))
    ...
    with torch.no_grad():
        ...
        for i in range(test_episodes):
            game_histories[i].init([init_obses[i] for _ in range(
                config.stacked_observations)], init_legal_actions[i])
        ...
        while not dones.all():
            ...
            # @wjc
            stack_legal_actions = [game_history.legal_actions[-1]
                                   for game_history in game_histories]
            ...
            if config.image_based:
                ...
                stack_obs = torch.from_numpy(np.array(stack_obs)).to(
                    device).reshape(test_episodes, -1)
            ...
            for i in range(test_episodes):
                ...
                action, _ = select_action(
                    distributions, temperature=1, deterministic=True, legal_actions=stack_legal_actions[i])
