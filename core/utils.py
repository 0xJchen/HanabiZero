...
class MaxAndSkipEnv(gym.Wrapper):
    def __init__(self, env, skip=4):
        ...
        self._obs_buffer = np.zeros((2,)+env.observation_space.shape, dtype=np.float)
        self._skip       = skip
        self.max_frame = np.zeros(env.observation_space.shape, dtype=np.float)
    ...
    def render(self, mode='human', **kwargs):
        img = self.max_frame
        img = cv2.resize(img, (400, 400), interpolation=cv2.INTER_AREA).astype(np.float)
        if mode == 'rgb_array':
            return img
        elif mode == 'human':
...
class WarpFrame(gym.ObservationWrapper):
    def __init__(self, env, width=84, height=84, grayscale=True, dict_space_key=None):
        ...
        assert original_space.dtype == np.float and len(original_space.shape) == 3
...
def str_to_arr(s, gray_scale=False):
    ...
    """
    nparr = np.frombuffer(s, np.float)
    if gray_scale:
